...
from safe_rl.policy.model.mlp_ac import (ValueNetIQL, EnsembleQCritic, CholeskyGaussianActor)
...
class CVPOIQL(Policy):
    def __init__(self,
                 ...
                 **kwargs) -> None:
        r'''
        Constrained Variational Policy Optimization equiped with IQL critic training
        ...
        value_c = ValueNetIQL(self.obs_dim, hidden_sizes, nn.ReLU)
        self._value_c_training_setup(value_c)
    ...
    def _value_c_training_setup(self, value_c):
        self.value_c = to_device(value_c, get_device_name())
        self.value_c_optimizer = Adam(self.value_c.parameters(), lr=self.critic_lr)
    ...
    def _update_value_c(self, data):
        ...
        def value_c_loss():
            ...
            with torch.no_grad():
                q, _ = self.critic_forward(self.qc_targ, obs, act)
            v = self.critic_forward(self.value_c, obs)
            assert v.shape == q.shape
            loss = self.value_c.loss(v - q, self.expectile).mean()  # - diff > 0: find promising data points with low cost
            v_info = {'Value_c': to_ndarray(v)}
            return loss, v_info
        
        self.value_c_optimizer.zero_grad()
        loss_v_c, loss_v_info = value_c_loss()
        loss_v_c.backward()
        self.value_c_optimizer.step()
        self.logger.store(LossV=loss_v_c.item(), **loss_v_info)
