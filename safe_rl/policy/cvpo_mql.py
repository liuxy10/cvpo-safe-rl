...
class CVPOMQL(Policy):
    def __init__(self,
        ...
        g = EnsembleQCritic(self.obs_dim,
                             self.act_dim,
                             self.hidden_sizes,
                             nn.ReLU,
                             num_q=1)
        self._g_training_setup(g)

        gc = EnsembleQCritic(self.obs_dim,
                             self.act_dim,
                             self.hidden_sizes,
                             nn.ReLU,
                             num_q=1)
        self._gc_training_setup(gc)
    ...
    def _g_training_setup(self, g):
        self.g = to_device(g, get_device_name())

        # Set up optimizers for safety critic
        self.g_optimizer = Adam(self.g.parameters(), lr=self.critic_lr)

    def _gc_training_setup(self, gc):
        self.gc = to_device(gc, get_device_name())

        # Set up optimizers for safety critic
        self.gc_optimizer = Adam(self.gc.parameters(), lr=self.critic_lr)
    ...
    def _update_critic(self, data):
        ...
        def critic_loss():
            ...
            with torch.no_grad():
                ...
                _, g_list = self.critic_forward(self.g, obs, act)
            ...
            loss_q = sum([((g_list[0] * (q - backup))**2).mean() for q in q_list])
    ...
    def _update_g(self, data):
        ...
        def g_loss():
            ...
            loss_g = - sum([((g_list[0] * (q - backup))**2).mean() for q in q_list])

            # Useful info for logging
            g_info = dict()
            for i, g in enumerate(g_list):
                g_info["GVals" + str(i)] = to_ndarray(g)
            return loss_g, g_info

        # First run one gradient descent step for Q1 and Q2
        self.g_optimizer.zero_grad()
        loss_g, loss_g_info = g_loss()
        loss_g.backward()
        self.g_optimizer.step()

        # Log g update info
        # Record things
        self.logger.store(LossG=loss_g.item(), **loss_g_info)
    ...
    def _update_qc(self, data):
        ...
        def critic_loss():
            ...
            with torch.no_grad():
                # Target actions come from *current* policy
                _, gc_list = self.critic_forward(self.gc, obs, act)
                _, logp_a_next, pi_dist = self.actor_forward(obs_next)
                act_next = pi_dist.sample()
                # Target Q-values
            ...
            # MSE loss against Bellman backup
            loss_q = sum([((gc_list[0] * (q - backup))**2).mean() for q in q_list])
    ...
    def _update_gc(self, data):
        ...
        def gc_loss():
            ...
            loss_gc = - sum([((gc_list[0] * (q - backup))**2).mean() for q in q_list])
            
            # Useful info for logging
            gc_info = dict()
            for i, gc in enumerate(gc_list):
                gc_info["GCVals" + str(i)] = to_ndarray(gc)
            return loss_gc, gc_info

        # First run one gradient descent step for Q1 and Q2
        self.gc_optimizer.zero_grad()
        loss_gc, loss_gc_info = gc_loss()
        loss_gc.backward()
        self.gc_optimizer.step()

        # Log critic update info
        # Record things
        self.logger.store(LossGC=loss_gc.item(), **loss_gc_info)
